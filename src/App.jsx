...
// ── VÍZKÉSZLET KALKULÁTOR ─────────────────────────────────────────────────
function calcWater(answers) {
  const sizeMap = {"40 m² alatt":35,"40–70 m²":55,"70–120 m²":95,"120–200 m²":160,"200 m² felett":220};
  const size = sizeMap[answers.r_size] || 80;

  // Roof area is roughly 60-70% of floor area for pitched roof
  const roofArea = Math.round(size * 0.65);

  // Regional rainfall map (mm/year) based on city/zip
  const city = (answers.r_city || "").toLowerCase();
  let rainfall = 550; // Hungarian average
  ...
  else if (city.includes("keszthely") || city.includes("hévíz")) rainfall = 700;

  // Runoff coefficient for roof (0.8 for tiles, 0.9 for metal)
  const runoff = answers.r_roof_type === "Lapostető (saját)" ? 0.85 : 0.80;

  // Annual collectible water in liters
  const annualLiters = Math.round(roofArea * rainfall * runoff);

  // Average Hungarian household water use: 150L/person/day
  const persons = {"1 fő":1,"2 fő":2,"3–4 fő":3.5,"5+ fő":5}[answers.r_persons] || 2;
  const annualUse = Math.round(persons * 150 * 365);

  // Toilet + garden use (can be replaced with rainwater)
  const replaceable = Math.round(annualUse * 0.40); // 40% non-potable use

  // Water cost savings (avg Hungarian water price ~350 Ft/m3)
  const savings = Math.round(Math.min(annualLiters, replaceable) * 0.35); // Ft

  // Recommended tank size (2 months supply)
  const tankSize = Math.round(Math.min(annualLiters, replaceable) / 6 / 1000) * 1000;

  // Drought risk
  let droughtRisk = "Közepes";
  let droughtColor = "#E67E22";
  ...
  return {
    ...
    replaceable,
    savings,
    tankSize: Math.max(tankSize, 3000),
    droughtRisk,
    droughtColor,
...
function WaterCard({ answers }) {
  ...
  if (!answers.r_roof_type || answers.r_roof_type === "Nincs saját tető (lakás)") return null;

  return (
    <div style={{ background:"linear-gradient(135deg,#EBF5FB,#E8F8F5)", border:"1.5px solid #A9D4F055", borderRadius:14, overflow:"hidden", marginBottom:12 }}>
      <div style={{ padding:"14px 16px", background:"linear-gradient(135deg,#1A5276,#1A7A5E)", display:"flex", alignItems:"center", gap:12 }}>
        <div style={{ fontSize:24 }}>💧</div>
        <div>
          <div style={{ fontWeight:800, fontSize:14, color:"#fff" }}>Vízgazdálkodási elemzés</div>
      ...
      <div style={{ padding:"14px 16px" }}>
        ...
        <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:14 }}>
          <div style={{ background:"#fff", borderRadius:10, padding:"12px 14px", flex:1, minWidth:100, border:"1px solid #A9D4F055" }}>
            <div style={{ fontSize:10, color:"#5D8AA8", fontWeight:700, marginBottom:4 }}>ÉVI GYŰJTHETŐ VÍZ</div>
            <div style={{ fontSize:20, fontWeight:800, color:"#1A5276" }}>{(w.annualLiters/1000).toFixed(0)}m³</div>
            <div style={{ fontSize:11, color:"#888" }}>{w.annualLiters.toLocaleString()} liter</div>
          </div>
          <div style={{ background:"#fff", borderRadius:10, padding:"12px 14px", flex:1, minWidth:100, border:"1px solid #A9D4F055" }}>
            <div style={{ fontSize:10, color:"#5D8AA8", fontWeight:700, marginBottom:4 }}>ÉVI MEGTAKARÍTÁS</div>
            <div style={{ fontSize:20, fontWeight:800, color:"#1A7A5E" }}>{formatFt(w.savings)}</div>
            <div style={{ fontSize:11, color:"#888" }}>vízdíj megtakarítás</div>
          </div>
          <div style={{ background:"#fff", borderRadius:10, padding:"12px 14px", flex:1, minWidth:100, border:"1px solid #A9D4F055" }}>
            <div style={{ fontSize:10, color:"#5D8AA8", fontWeight:700, marginBottom:4 }}>ÖNELLÁTÁS</div>
            <div style={{ fontSize:20, fontWeight:800, color:"#1A5276" }}>{w.selfSufficiency}%</div>
            <div style={{ fontSize:11, color:"#888" }}>nem ivóvíz igény</div>
          </div>
        </div>

        {/* Rainfall bar */}
        <div style={{ background:"#fff", borderRadius:10, padding:"12px 14px", marginBottom:10, border:"1px solid #A9D4F055" }}>
          ...
          <div style={{ display:"flex", justifyContent:"space-between", marginTop:4 }}>
            <span style={{ fontSize:10, color:"#888" }}>400mm (száraz)</span>
            <span style={{ fontSize:10, color:"#888" }}>800mm (csapadékos)</span>
          </div>
        </div>
        ...
        <div style={{ background:"#fff", borderRadius:10, padding:"12px 14px", marginBottom:12, border:"1px solid #A9D4F055" }}>
          <div style={{ fontSize:12, fontWeight:700, color:"#1A5276", marginBottom:8 }}>💡 Mit vált ki az esővíz?</div>
          {[
            { icon:"🚽", label:"WC öblítés", pct:30 },
            { icon:"🌱", label:"Kerti locsolás", pct:25 },
            { icon:"🧺", label:"Mosógép", pct:15 },
            { icon:"🚗", label:"Autómosás, takarítás", pct:10 },
          ].map(item => (
            <div key={item.label} style={{ display:"flex", alignItems:"center", gap:8, marginBottom:6 }}>
              <span style={{ fontSize:14, width:20 }}>{item.icon}</span>
              <span style={{ fontSize:12, color:"#555", flex:1 }}>{item.label}</span>
              <div style={{ width:80, height:6, background:"#E8F4FD", borderRadius:3, overflow:"hidden" }}>
                <div style={{ height:"100%", width:`${item.pct/30*100}%`, background:"#3498DB", borderRadius:3 }} />
              </div>
              <span style={{ fontSize:11, color:"#3498DB", fontWeight:700, width:30, textAlign:"right" }}>{item.pct}%</span>
            </div>
          ))}
        </div>
