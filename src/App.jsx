...
// House + Leaf SVG logo icon
function HouseLeafIcon({ size = 32, color = "#fff" }) {
  return (
    <svg width={size} height={size} viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M4 14L16 4L28 14V28H20V20H12V28H4V14Z" stroke={color} strokeWidth="2" strokeLinejoin="round" fill="none"/>
      <path d="M13 17C13 17 14 13 18 13C18 13 18 17 15 19C15 19 17 19 19 17" stroke={color} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  );
}
...
const BLOCKS = {
  residential: [
    ...
    { id:"kornyezet", label:"Környezet", color:C.primaryDark },
    { id:"celok", label:"Célok", color:C.primary },
  ],
  commercial: [
    { id:"epulet", label:"Épület", color:C.blue },
    { id:"fogyasztas", label:"Fogyasztás", color:C.orange },
...
function getResidentialRecs(answers) {
  ...
  if (own && hasRoof && goodDir && noSolar) recs.push({ priority:poorWallIns ? 2 : 1, name:"Napelem rendszer", tag:highElec ? "KIEMELT" : "AJÁNLOTT", tagColor:C.primary, cost:"1 500 000 – 4 000 000 Ft", payback:"5–8 év" });
  if (answers.r_hotwater !== "Napkollektor" && own && hasRoof) recs.push({ priority:2, name:"Napkollektor (melegvíz)", tag:"GYORS MEGTÉRÜLÉS", tagColor:C.primaryDark, cost:"400 000 – 900 000 Ft", payback:"4–7 év" });
  ...
  if (wantsIndep && bigBudget) recs.push({ priority:4, name:"Akkumulátor rendszer", tag:"AUTONÓMIA", tagColor:C.primaryDark, cost:"1 500 000 – 3 500 000 Ft", payback:"8–12 év" });
  ...
  if (recs.length === 0) recs.push({ priority:1, name:"Okos termosztát + mérés", tag:"AZONNAL", tagColor:C.primaryDark, cost:"30 000 – 150 000 Ft", payback:"1–2 év", confidence:85 });
...
function getCommercialRecs(answers) {
  ...
  if (own && hasRoof && goodDir && noSolar) recs.push({ priority:poorIns ? 2 : 1, name:"Ipari / kereskedelmi napelem", tag:highElec ? "KIEMELT" : "AJÁNLOTT", tagColor:C.primary, cost:"3 000 000 – 20 000 000 Ft", payback:"4–7 év" });
  ...
  if (bigBudget) recs.push({ priority:4, name:"Akkumulátor (kereskedelmi)", tag:"CSÚCSTELJ. KEZELÉS", tagColor:C.primaryDark, cost:"5 000 000 – 30 000 000 Ft", payback:"6–10 év" });
  ...
  if (recs.length === 0) recs.push({ priority:1, name:"Energiaaudit + mérés", tag:"ELSŐ LÉPÉS", tagColor:C.primaryDark, cost:"150 000 – 400 000 Ft", payback:"Azonnal" });
    ...
    <div style={{ display:"flex", gap:4, marginBottom:24, overflowX:"auto", paddingBottom:4 }}>
      {blockCompletion.map(b => (
        <div key={b.id} style={{ flex:1, minWidth:48 }}>
          <div style={{ fontSize:9, color: b.pct > 0 ? b.color : C.muted, fontWeight:700, textAlign:"center", marginBottom:4, letterSpacing:0.5 }}>{b.label}</div>
...
function QuizOption({ label, selected, onClick, multi }) {
  ...
  return (
    <button
      ...
      style={{
        padding:"13px 16px",
        background: selected ? C.primaryLight : hover ? "#F9FBF9" : C.white,
        border:`1.5px solid ${selected ? C.primary : hover ? C.primary+"66" : C.grayMid}`,
        borderRadius:8,
        cursor:"pointer",
        textAlign:"left",
        fontSize:14,
    ...
    >
      <div style={{
        ...
        border:`2px solid ${selected ? C.primary : C.grayMid}`,
...
function generatePDF(answers, flow, currentRating, improvedRat, recs, contact = {}) {
  ...
  const recRows = recs.map(r =>
    `<tr>
      <td style="padding:10px 14px;font-size:13px;border-bottom:1px solid #f0f0f0;"><strong>${r.name}</strong></td>
      <td style="padding:10px 14px;font-size:13px;color:#555;border-bottom:1px solid #f0f0f0;">${r.cost}</td>
      <td style="padding:10px 14px;font-size:13px;color:#555;border-bottom:1px solid #f0f0f0;">${r.payback}</td>
    </tr>`
  ).join("");
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body { font-family: 'Poppins', Arial, sans-serif; background: #fff; color: #1E1E1E; }
  ...
  .header-right .doc-title { font-size: 13px; font-weight: 700; }
  ...
  .rating-box-title { font-size: 11px; font-weight: 800; color: #2E7D32; letter-spacing: 1.5px; text-transform: uppercase; margin-bottom: 18px; }
...
<div class="page">
    ...
    <div class="logo-wrap">
      <div class="logo-icon">
        <svg width="28" height="28" viewBox="0 0 32 32" fill="none">
          <path d="M4 14L16 4L28 14V28H20V20H12V28H4V14Z" stroke="white" stroke-width="2.5" stroke-linejoin="round" fill="none"/>
          <path d="M13 17C13 17 14 13 18 13C18 13 18 17 15 19C15 19 17 19 19 17" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>
      </div>
      <div>
        <div class="logo-name"><span style="font-weight:300">re</span><span style="font-weight:700">Source</span> <span style="font-size:14px;color:#999;font-weight:400">app</span></div>
  ...
  <div class="footer">
    <div class="footer-left">
      <div class="name">reSource app</div>
      <div class="contact">hello@resourcestrategist.com · resourcestrategist.com</div>
    </div>
    <div class="footer-right">© 2025 reSource</div>
  </div>
  <div class="disclaimer">* Ez az összefoglaló tájékoztató jellegű, a megadott válaszok alapján becsült értékekkel dolgozik. Pontos energetikai tanúsítványhoz tanúsító szakember bevonása szükséges.</div>
  <div class="print-btn-wrap no-print">
    <button class="print-btn" onclick="window.print()">Mentés PDF-ként</button>
  </div>
</div>
</body>
</html>`;
...
function ResultsView({ answers, flow, onRestart, detailedMode, setDetailedMode, setStep, setScreen }) {
  ...
  return (
    <div>
      {/* Header */}
      <div style={{ background:C.primaryLight, border:`1.5px solid ${C.primary}44`, borderRadius:12, padding:"20px 18px", marginBottom:18 }}>
        <h2 style={{ fontSize:20, fontWeight:700, color:C.text, marginBottom:6 }}>Épületed energetikai terve</h2>
      ...
      <div style={{ background:C.white, border:`1.5px solid ${C.grayMid}`, borderRadius:12, overflow:"hidden", marginBottom:14 }}>
        <div style={{ background:C.primaryLight, padding:"14px 16px", borderBottom:`1px solid ${C.grayMid}` }}>
          <div style={{ fontSize:11, fontWeight:700, color:C.primaryDark, letterSpacing:1.5, textTransform:"uppercase" }}>Energetikai besorolás</div>
        </div>
        <div style={{ padding:"16px" }}>
          <RatingBar label="Jelenlegi besorolás" rating={current} />
          <div style={{ textAlign:"center", fontSize:16, color:C.primary, margin:"4px 0", fontWeight:600 }}>↓</div>
          <RatingBar label="Felújítás utáni besorolás" rating={improved} />
          {steps > 0 && (
            <div style={{ marginTop:10, background:C.primaryLight, border:`1px solid ${C.primary}44`, borderRadius:8, padding:"8px 12px", fontSize:13, color:C.primaryDark, fontWeight:600, textAlign:"center" }}>
              {steps} kategóriás javulás érhető el
            </div>
          )}
      ...
      {/* TOP 1 */}
      {recs[0] && (
        <div style={{ background:C.dark, borderRadius:12, padding:"20px 18px", marginBottom:14 }}>
          <div style={{ fontSize:10, fontWeight:700, color:C.primary, letterSpacing:2, marginBottom:8, textTransform:"uppercase" }}>Ha csak 1 dolgot teszel</div>
          <div style={{ fontSize:19, fontWeight:700, color:"#fff", marginBottom:6 }}>{recs[0].name}</div>
          <div style={{ fontSize:13, color:"#bbb", lineHeight:1.6, marginBottom:12 }}>Ez a legfontosabb lépés a te épületed esetében. Minden más erre épül.</div>
          {recs[0].roi && (
            <div style={{ display:"flex", gap:8 }}>
              <div style={{ background:"#ffffff14", borderRadius:8, padding:"10px 12px", flex:1 }}>
                <div style={{ fontSize:10, color:"#888", marginBottom:2, textTransform:"uppercase", letterSpacing:0.5 }}>Évi megtakarítás</div>
                <div style={{ fontWeight:700, fontSize:14, color:C.primary }}>{formatFt(recs[0].roi.save)}</div>
              </div>
              <div style={{ background:"#ffffff14", borderRadius:8, padding:"10px 12px", flex:1 }}>
                <div style={{ fontSize:10, color:"#888", marginBottom:2, textTransform:"uppercase", letterSpacing:0.5 }}>Megtérülés</div>
                <div style={{ fontWeight:700, fontSize:14, color:C.primary }}>{recs[0].roi.years} év</div>
              </div>
            </div>
          )}
        </div>
      ...
      {recs.map((rec, i) => (
        <div key={rec.name} style={{ background:C.white, border:`1.5px solid ${C.grayMid}`, borderRadius:10, padding:"14px 16px", marginBottom:10 }}>
          <div style={{ display:"flex", alignItems:"flex-start", gap:12 }}>
            <div style={{ width:6, height:6, borderRadius:"50%", background:rec.tagColor, marginTop:6, flexShrink:0 }} />
            <div style={{ flex:1 }}>
              <div style={{ marginBottom:4 }}>
                <span style={{ fontSize:10, fontWeight:700, color:rec.tagColor, background:rec.tagColor+"18", padding:"2px 8px", borderRadius:4, letterSpacing:0.5 }}>{rec.tag}</span>
              </div>
              <div style={{ fontWeight:600, fontSize:14, color:C.text, marginBottom:4 }}>{rec.name}</div>
              {rec.confidence && (
                <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom: rec.roi ? 10 : 0 }}>
                  <div style={{ flex:1, height:3, background:C.grayMid, borderRadius:2, overflow:"hidden" }}>
                    <div style={{ height:"100%", width:`${rec.confidence}%`, background: rec.confidence > 80 ? C.primary : rec.confidence > 60 ? C.orange : C.red, borderRadius:2 }} />
                  </div>
              ...
              {rec.roi && (
                <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                  <div style={{ background:C.grayLight, borderRadius:6, padding:"7px 10px", flex:1, minWidth:80 }}>
                    <div style={{ fontSize:9, color:C.muted, marginBottom:2, textTransform:"uppercase", letterSpacing:0.5 }}>Beruházás</div>
                    <div style={{ fontWeight:600, fontSize:12, color:C.text }}>{formatFt(rec.roi.cost)}</div>
                  </div>
                  <div style={{ background:C.primaryLight, borderRadius:6, padding:"7px 10px", flex:1, minWidth:80 }}>
                    <div style={{ fontSize:9, color:C.primaryDark, marginBottom:2, textTransform:"uppercase", letterSpacing:0.5 }}>Évi megtakarítás</div>
                    <div style={{ fontWeight:600, fontSize:12, color:C.primaryDark }}>{formatFt(rec.roi.save)}</div>
                  </div>
                  <div style={{ background:"#E3F2FD", borderRadius:6, padding:"7px 10px", flex:1, minWidth:80 }}>
                    <div style={{ fontSize:9, color:C.blue, marginBottom:2, textTransform:"uppercase", letterSpacing:0.5 }}>Megtérülés</div>
                    <div style={{ fontWeight:600, fontSize:12, color:C.blue }}>{rec.roi.years} év</div>
                  </div>
                </div>
              )}
              {!rec.roi && (
                <div style={{ fontSize:12, color:C.muted }}>{rec.cost} · {rec.payback}</div>
              )}
            </div>
          </div>
        </div>
      ...
      {/* Golden rule */}
      <div style={{ background:C.grayLight, borderRadius:10, padding:"14px 16px", margin:"14px 0 10px" }}>
      ...
      {/* Részletes felmérés */}
      {!detailedMode && (
        <div style={{ background:C.primaryLight, border:`1.5px solid ${C.primary}44`, borderRadius:12, padding:"16px", marginBottom:10 }}>
          ...
          <button onClick={() => { setDetailedMode(true); setStep(14); setScreen("quiz"); }}
            style={{ width:"100%", padding:"12px", background:C.primary, border:"none", borderRadius:8, cursor:"pointer", fontWeight:700, fontSize:14, color:"#fff", fontFamily:"'Poppins', sans-serif" }}>
      ...
      <div style={{ background:C.white, border:`1.5px solid ${C.grayMid}`, borderRadius:12, overflow:"hidden", marginBottom:12 }}>
        <div style={{ padding:"16px", background:C.primaryLight, borderBottom:`1px solid ${C.grayMid}` }}>
          <div style={{ fontSize:13, fontWeight:700, color:C.text, marginBottom:2 }}>PDF összefoglaló letöltése</div>
          <div style={{ fontSize:12, color:C.muted }}>Add meg adataidat – emailben is elküldjük.</div>
        </div>
        <div style={{ padding:"16px" }}>
          {!contactDone ? (
            <>
              <div style={{ display:"flex", flexDirection:"column", gap:10, marginBottom:10 }}>
                <input placeholder="Neved *" value={contact.name} onChange={e => setContact(p => ({...p, name:e.target.value}))} style={{ ...inputStyle, borderColor: contactError && !contact.name.trim() ? C.red : C.grayMid }} />
                <input placeholder="Irányítószám és város *" value={contact.city} onChange={e => setContact(p => ({...p, city:e.target.value}))} style={{ ...inputStyle, borderColor: contactError && !contact.city?.trim() ? C.red : C.grayMid }} />
                <input placeholder="Utca, házszám" value={contact.street} onChange={e => setContact(p => ({...p, street:e.target.value}))} style={inputStyle} />
                <input placeholder="Email cím" value={contact.email} onChange={e => setContact(p => ({...p, email:e.target.value}))} style={inputStyle} />
                <input placeholder="Telefonszám" value={contact.phone} onChange={e => setContact(p => ({...p, phone:e.target.value}))} style={inputStyle} />
              </div>
              {contactError && <div style={{ fontSize:12, color:C.red, marginBottom:8 }}>{contactError}</div>}
              <button onClick={handleContactSubmit} style={{ width:"100%", padding:"13px", background:C.primary, border:"none", borderRadius:8, cursor:"pointer", fontWeight:700, fontSize:14, color:"#fff", fontFamily:"'Poppins', sans-serif" }}>
            ...
            <>
              <div style={{ background:C.primaryLight, borderRadius:8, padding:"10px 14px", marginBottom:12, fontSize:13, color:C.primaryDark, fontWeight:600 }}>
                Köszönöm, {contact.name}!
              </div>
              <button onClick={handleDownload} disabled={downloading}
                style={{ width:"100%", padding:"13px", background:downloading ? C.grayMid : C.primary, border:"none", borderRadius:8, cursor:downloading ? "default" : "pointer", fontWeight:700, fontSize:14, color:"#fff", fontFamily:"'Poppins', sans-serif" }}>
      ...
      <button onClick={onRestart} style={{ width:"100%", padding:"12px", background:"transparent", border:`1.5px solid ${C.grayMid}`, borderRadius:8, cursor:"pointer", fontSize:13, color:C.muted, fontWeight:500, fontFamily:"'Poppins', sans-serif" }}>
...
export default function ResourceApp() {
  ...
  const handleRestart = () => {
    setAnswers({}); setStep(0); setSelected([]); setFlow(null); setDetailedMode(false); setScreen("intro");
  };

  const baseFont = "'Poppins', 'Helvetica Neue', Arial, sans-serif";

  return (
    <div style={{ minHeight:"100vh", background:C.grayLight, fontFamily:baseFont, display:"flex", alignItems:"flex-start", justifyContent:"center", padding:"24px 16px 60px" }}>
      <div style={{ width:"100%", maxWidth:480 }}>

        {/* Logo */}
        <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:20 }}>
          <div style={{ width:40, height:40, background:C.primary, borderRadius:10, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
            <HouseLeafIcon size={24} color="#fff" />
          </div>
          <div>
            <div style={{ fontFamily:baseFont, fontSize:19, fontWeight:700, color:C.text, lineHeight:1 }}>
              <span style={{ fontWeight:300 }}>re</span><span style={{ fontWeight:700 }}>Source</span>
            </div>
            <div style={{ fontSize:11, color:C.muted, letterSpacing:"0.5px", marginTop:2, fontWeight:400 }}>app</div>
          </div>
        </div>
        ...
        <div style={{ background:C.white, borderRadius:16, padding:"26px 22px", boxShadow:"0 2px 20px rgba(0,0,0,0.06)" }}>
          ...
          {screen === "intro" && (
            <div>
              <div style={{ width:56, height:56, background:C.primaryLight, borderRadius:14, display:"flex", alignItems:"center", justifyContent:"center", marginBottom:16 }}>
                <HouseLeafIcon size={32} color={C.primary} />
              </div>
              ...
              <button onClick={() => setScreen("flowSelect")} style={{ width:"100%", padding:"14px", background:C.primary, border:"none", borderRadius:10, cursor:"pointer", fontSize:15, fontWeight:700, color:"#fff", fontFamily:baseFont }}>
                Felmérés indítása
              </button>
            </div>
          )}
          ...
          {screen === "flowSelect" && (
            <div>
              ...
              {[
                { id:"residential", label:"Lakóépület", desc:"Ház, lakás, tanya, nyaraló", color:C.primary },
                { id:"commercial", label:"Vállalkozás / Üzlet", desc:"Iroda, üzlet, üzem, raktár", color:C.teal },
              ].map(f => (
                <button key={f.id} onClick={() => { setFlow(f.id); setStep(0); setAnswers({}); setScreen("quiz"); }}
                  style={{ width:"100%", padding:"16px", background:C.white, border:`1.5px solid ${C.grayMid}`, borderRadius:12, cursor:"pointer", textAlign:"left", marginBottom:10, display:"flex", alignItems:"center", gap:14, fontFamily:baseFont, transition:"all 0.15s" }}
                  onMouseEnter={e => { e.currentTarget.style.borderColor = f.color; e.currentTarget.style.background = "#F9FDF9"; }}
                  onMouseLeave={e => { e.currentTarget.style.borderColor = C.grayMid; e.currentTarget.style.background = C.white; }}
                >
                  <div style={{ width:44, height:44, borderRadius:10, background:f.color+"18", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
                    <HouseLeafIcon size={24} color={f.color} />
                  </div>
                  <div>
              ...
              <button onClick={() => setScreen("intro")} style={{ background:"none", border:"none", color:C.muted, fontSize:13, cursor:"pointer", padding:"8px 0", fontFamily:baseFont }}>← Vissza</button>
            </div>
          )}
            ...
            <div>
              ...
              <div style={{ marginBottom:20 }}>
                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
                  ...
                  <span style={{ fontSize:11, color:C.primaryDark, fontWeight:700 }}>{pct}%</span>
                </div>
                <div style={{ height:3, background:C.grayMid, borderRadius:2, overflow:"hidden" }}>
                  <div style={{ height:"100%", width:`${pct}%`, background:C.primary, borderRadius:2, transition:"width 0.3s" }} />
                </div>
              </div>
              {currentBlock && (
              ...
              {isFreetext ? (
                <>
                  <textarea value={freetext} onChange={e => setFreetext(e.target.value)} placeholder="Ide írhatod a megjegyzésedet…" rows={4}
                    style={{ width:"100%", padding:"12px 14px", border:`1.5px solid ${C.grayMid}`, borderRadius:8, fontSize:14, color:C.text, resize:"vertical", outline:"none", fontFamily:baseFont }} />
                  <button onClick={confirmFreetext} style={{ marginTop:12, width:"100%", padding:"13px", background:C.primary, border:"none", borderRadius:8, cursor:"pointer", fontWeight:700, fontSize:14, color:"#fff", fontFamily:baseFont }}>
                ...
                <>
                  ...
                  {isMulti && (
                    <button onClick={confirmMulti} disabled={selected.length === 0}
                      style={{ marginTop:14, width:"100%", padding:"13px", background:selected.length > 0 ? C.primary : C.grayMid, border:"none", borderRadius:8, cursor:selected.length > 0 ? "pointer" : "default", fontWeight:700, fontSize:14, color:selected.length > 0 ? "#fff" : C.muted, fontFamily:baseFont }}>
                      Tovább
                    </button>
                  )}
                </>
              )}
              <button onClick={handleBack} style={{ marginTop:14, background:"none", border:"none", color:C.muted, fontSize:13, cursor:"pointer", padding:0, fontFamily:baseFont }}>← Vissza</button>
            </div>
          )}
