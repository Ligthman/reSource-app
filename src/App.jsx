...
const QUESTIONS = {
  residential: [
    // ÉPÜLET
    { id:"r_type",       block:"epulet",    q:"Milyen típusú az épület?",                              multi:false, opts:["Önálló családi ház","Ikerház","Sorház","Társasházi lakás – téglaépület","Panellakás / panel épület","Tanya / vidéki birtok","Nyaraló"] },
    ...
    { id:"r_size",       block:"epulet",    q:"Mekkora az alapterület?",                               multi:false, opts:["40 m² alatt","40–70 m²","70–120 m²","120–200 m²","200 m² felett"] },
    ...
    { id:"r_year",       block:"epulet",    q:"Mikor épült az épület?",                                multi:false, opts:["1960 előtt","1960–1980","1980–2000","2000–2010","2010 után"] },
    { id:"r_material",   block:"epulet",    q:"Mi az épület fő falanyaga?",                            multi:false, opts:["Tégla – régi tömör","Tégla – modern üreges","Ytong / pórobeton","Beton / panel","Fa szerkezet","Vályog","Kő","Vegyes / nem tudom"] },
    { id:"r_roof_type",  block:"epulet",    q:"Milyen a tető típusa?",                                 multi:false, opts:["Nyeregtető (saját)","Kontyolt tető (saját)","Lapostető (saját)","Nincs saját tető (lakás)"] },
    ...
    { id:"r_windows",    block:"epulet",    q:"Milyen típusú a nyílászáró?",                           multi:false, opts:["Egyrétegű – fa keret (régi)","Kétrétegű – régebbi PVC/fa","Kétrétegű – modern, jó tömítés","Háromrétegű (korszerű)","Vegyes"] },
    ...
    // FOGYASZTÁS
    { id:"r_persons",    block:"fogyasztas", q:"Hány személy él az épületben?",                        multi:false, opts:["1 fő","2 fő","3–4 fő","5+ fő"] },
    { id:"r_heating",    block:"fogyasztas", q:"Mivel fűtöd az épületet? (több is lehetséges)",        multi:true,  opts:["Gázkazán","Kombi cirkó","Távhő (szolgáltatói)","Elektromos fűtőtest","Hőszivattyú","Fa / pellet kazán","Kandalló","Padlófűtés","Egyéb"] },
    ...
    { id:"r_hotwater",   block:"fogyasztas", q:"Honnan jön a melegvíz?",                               multi:false, opts:["Kombi cirkó (bojler nélkül)","Gázboiler","Elektromos bojler","Távhő (szolgáltatói)","Napkollektor","Hőszivattyú","Egyéb"] },
    ...
    { id:"r_gasbill",    block:"fogyasztas", q:"Havi átlag gázszámla (fűtési szezonban)?",             multi:false, opts:["Nincs gáz","0–15 000 Ft","15 000–40 000 Ft","40 000–80 000 Ft","80 000 Ft felett"] },
    { id:"r_elecbill",   block:"fogyasztas", q:"Havi átlag villanyszámla?",                            multi:false, opts:["0–10 000 Ft","10 000–25 000 Ft","25 000–50 000 Ft","50 000 Ft felett"] },
    ...
    // MEGLÉVŐ RENDSZEREK
    { id:"r_solar_pv",   block:"rendszerek", q:"Van-e már napelem?",                                   multi:false, opts:["Igen, van","Nincs","Tervezett / folyamatban"] },
    ...
    { id:"r_goal",       block:"celok",      q:"Mi a fő motiváció? (több is)",                         multi:true,  opts:["Spórolni a számlákon","Energetikai függetlenség","Környezettudatosság","Ingatlan értéke","Komfort növelése"] },
    { id:"r_budget",     block:"celok",      q:"Mekkora tőke áll rendelkezésre?",                      multi:false, opts:["0–500 000 Ft","500 000 – 2 000 000 Ft","2 000 000 – 5 000 000 Ft","5 000 000 Ft felett"] },
...
function ResultsView({ answers, flow, onRestart }) {
  ...
  return (
    <div>
      ...
      {/* Complex Home */}
      <div style={{ background:"linear-gradient(135deg,#f0faf018,#e8f5e918)", border:"1.5px solid #4CAF8244", borderRadius:14, overflow:"hidden", marginBottom:12 }}>
        <div style={{ padding:"14px 16px", background:"linear-gradient(135deg,#2a9a2a,#6abf3d)", display:"flex", alignItems:"center", gap:12 }}>
          <div style={{ fontSize:24 }}>🌿</div>
          <div>
            <div style={{ fontWeight:800, fontSize:14, color:"#fff" }}>Complex Home – Ha újat építenél</div>
            <div style={{ fontSize:11, color:"#d0f0d0", marginTop:1 }}>Moduláris, rezsimentes, önellátó passzívház</div>
          </div>
        </div>
        <div style={{ padding:"14px 16px", background:"#fff" }}>
          <p style={{ fontSize:13, color:C.gray, lineHeight:1.65, marginBottom:12 }}>Favázas, előre gyártott elemekből összerakható, teljesen off-grid ház – napelemmel, hőszivattyúval, esővízgyűjtéssel. 3 embernek ~30 nap az összerakás. Megfizethetően, bárhol a világon.</p>
          <div style={{ display:"flex", gap:7, flexWrap:"wrap", marginBottom:14 }}>
            {["🌿 Rezsimentes","⚡ Off-grid","🔧 ~30 nap","🌍 Bárhol","♻️ Fenntartható"].map(t => (
              <span key={t} style={{ fontSize:11, background:"#e8f5e9", color:"#2a7a2a", borderRadius:6, padding:"3px 9px", fontWeight:700 }}>{t}</span>
            ))}
          </div>
          <button style={{ width:"100%", padding:"12px", background:"linear-gradient(135deg,#6abf3d,#2a9a2a)", border:"none", borderRadius:10, cursor:"pointer", fontWeight:800, fontSize:14, color:"#fff" }}>
            Többet tudok a Complex Home-ról →
          </button>
        </div>
      </div>
      ...
      <div style={{ borderRadius:14, overflow:"hidden", border:`1.5px solid ${C.grayMid}`, marginBottom:14 }}>
        <div style={{ padding:"16px 16px", background:C.white }}>
          ...
          <p style={{ fontSize:13, color:C.gray, lineHeight:1.6, marginBottom:14 }}>Felújítás vagy új ház – segítünk megtalálni a legjobb utat. A Resource App és a Complex Home csapata együtt áll rendelkezésedre.</p>
...
export default function ResourceApp() {
  ...
  const handleRestart = () => { setAnswers({}); setStep(0); setSelected([]); setFlow(null); setScreen("intro"); };
  return (
    <div style={{ minHeight:"100vh", background:C.grayLight, fontFamily:"'Helvetica Neue', Arial, sans-serif", display:"flex", alignItems:"flex-start", justifyContent:"center", padding:"24px 16px 60px" }}>
      <div style={{ width:"100%", maxWidth:480 }}>
        ...
        <div style={{ textAlign:"center", marginTop:14, fontSize:11, color:C.muted }}>Resource App · Complex Home partnerrel · 2025</div>
      </div>
    </div>
  );
}
