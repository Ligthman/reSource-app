...
`<tr> <td style="padding:10px 14px;font-size:13px;border-bottom:1px solid #f0ede8;">${r.icon} <strong>${r.name}</strong></td> <td style="padding:10px 14px;font-size:13px;color:#555;border-bottom:1px solid #f0ede8;">${r.cost}</td> <td style="padding:10px 14px;font-size:13px;color:#555;border-bottom:1px solid #f0ede8;">${r.payback}</td> </tr>`
).join("");
...
<input placeholder="Neved *" value={contact.name} onChange={e => setContact(p => ({...p, name: e.target.value}))} style={{ padding:"11px 14px", border:`1.5px solid ${contactError && !contact.name.trim() ? C.red : C.grayMid}`, borderRadius:10, fontSize:14, outline:"none", color:C.text, fontFamily:"inherit" }} />
<input placeholder="Irányítószám és város *" value={contact.city} onChange={e => setContact(p => ({...p, city: e.target.value}))} style={{ padding:"11px 14px", border:`1.5px solid ${contactError && !contact.city?.trim() ? C.red : C.grayMid}`, borderRadius:10, fontSize:14, outline:"none", color:C.text, fontFamily:"inherit" }} />
<input placeholder="Utca, házszám" value={contact.street} onChange={e => setContact(p => ({...p, street: e.target.value}))} style={{ padding:"11px 14px", border:`1.5px solid ${C.grayMid}`, borderRadius:10, fontSize:14, outline:"none", color:C.text, fontFamily:"inherit" }} />
<input placeholder="Email cím" value={contact.email} onChange={e => setContact(p => ({...p, email: e.target.value}))} style={{ padding:"11px 14px", border:`1.5px solid ${contactError && !contact.email && !contact.phone ? C.red : C.grayMid}`, borderRadius:10, fontSize:14, outline:"none", color:C.text, fontFamily:"inherit" }} />
<input placeholder="Telefonszám" value={contact.phone} onChange={e => setContact(p => ({...p, phone: e.target.value}))} style={{ padding:"11px 14px", border:`1.5px solid ${C.grayMid}`, borderRadius:10, fontSize:14, outline:"none", color:C.text, fontFamily:"inherit" }} />
</div>
{contactError && <div style={{ fontSize:12, color:C.red, marginBottom:8 }}>⚠ {contactError}</div>}
<button onClick={handleContactSubmit} style={{ width:"100%", padding:"13px", background:C.sun, border:"none", borderRadius:10, cursor:"pointer", fontWeight:800, fontSize:15, color:C.text }}>
...
<button onClick={handleDownload} disabled={downloading} style={{ width:"100%", padding:"13px", background:downloading ? C.grayMid : C.sun, border:"none", borderRadius:10, cursor:downloading ? "default" : "pointer", fontWeight:800, fontSize:15, color:C.text, display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
